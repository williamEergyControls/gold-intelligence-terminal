/* ================================================================
   HOME PORTAL RUNTIME — profile, real inflation, rates, AI w/ profile
   ================================================================ */
(function () {
'use strict';
...
const bootEl = $('#boot');
bootEl.addEventListener('click', () => { bootEl.classList.add('off'); setTimeout(() => bootEl.remove(), 500); });
(function seq() {
  const box = $('#bootLines'); const L = ['GIT/4.2.26 — HOME', '', 'PROFILE .......... LOADED', 'FEEDS ............ CONNECTING', 'RATES ............ FRED', '', 'READY.'];
  let i = 0; const t = setInterval(() => {
    if (i >= L.length) { clearInterval(t); bootEl.classList.add('off'); setTimeout(() => bootEl.remove(), 500); return; }
...
let P = Object.assign({}, DEF, (() => { try { return JSON.parse(localStorage.getItem('git-profile') || '{}'); } catch { return {}; } })());
const save = () => localStorage.setItem('git-profile', JSON.stringify(P));
 $('#opname').value = P.name;
 $('#opname').addEventListener('input', () => { P.name = $('#opname').value.slice(0, 24); save(); });
['wf', 'we', 'wi', 'wh', 'wg'].forEach(id => {
  const el = $('#' + id); el.value = P[id]; el.nextElementSibling.textContent = P[id] + '%';
...
function calc() {
  ...
  const g = $('#riGap'); g.textContent = 'GAP ' + (gap > 0 ? '+' : '') + fmt(gap, 1) + 'pp — ' + (gap > 0.5 ? 'REALITY RUNS HOTTER THAN OFFICIAL' : gap < -0.5 ? 'REALITY COOLER THAN OFFICIAL' : 'IN LINE WITH OFFICIAL');
  g.className = 'cv ' + (gap > 0 ? 'dn' : 'up'); g.style.fontSize = '11px'; g.style.fontWeight = '600';
...
function renderGold() {
  ...
  const bc = $('#pxc'); bc.className = 'bigchg ' + (p >= 0 ? 'up' : 'dn');
  ...
  const m = B.ml;
  $('#mlLine').innerHTML = m
    ? `ML SIGNAL ▸ <b class="${m.p >= 0.5 ? 'up' : 'dn'}">${m.direction} ${(m.p * 100).toFixed(0)}%</b> · REGIME <b class="gold">${esc(m.regime.state)}</b> · AGREE ${m.final.agreeing}/10 · ACC ${esc(m.final.accStatus)} · <a href="/gold.html" style="color:var(--cyan)">FULL DESK ↗</a>`
...
function renderRates() {
  ...
  let h = keys.map(k => {
    ...
    return `<div class="ri"><span class="n">${esc(r.label)}</span><span class="gold">${fmt(r.value, 2)}%</span><span class="${d <= 0 ? 'up' : 'dn'}" style="justify-self:end;font-size:9px">${d <= 0 ? '▼' : '▲'}${fmt(Math.abs(d), 1)}bp</span></div>`;
  }).join('');
...
function renderHealth() {
  ...
  $('#hBody').innerHTML = B.health.map(h => `${h.status === 'online' ? '<span class="up">●</span>' : '<span class="dim">○</span>'} ${esc(h.name.toUpperCase())} — ${esc(h.status.toUpperCase())}`).join('<br>');
}
function renderTape() {
  const h = (B.tape || []).map(t => {
    const ch = t.changePct ?? 0, dg = t.price > 500 ? 1 : (t.price > 20 ? 2 : 3);
    return `<span class="tg"><span class="k">${esc(t.symbol)}</span><span class="v">${fmt(t.price, dg)}</span><span class="c ${ch >= 0 ? 'up' : 'dn'}">${ch >= 0 ? '▲' : '▼'}${sgn(ch)}${fmt(Math.abs(ch), 2)}%</span></span>`;
  }).join('');
  $('#tapeA').innerHTML = h; $('#tapeB').innerHTML = h;
}
function renderAll() { renderTape(); renderGold(); renderRates(); renderNews(); renderHealth(); calc(); }

/* ---------- AI analyst WITH profile ---------- */
const aOut = $('#aOut');
...
(async function init() {
  ...
  catch (e) { line ? null : null; const s = $('#status .mid'); if (s) s.textContent = 'API ERROR — ' + String(e && e.message || e); }
})();
setInterval(async () => {
  if (document.hidden || !B) return;
  try { B = await getJSON('/api/bootstrap?tf=1D'); renderAll(); } catch { }
}, 60000);
setInterval(async () => {
  if (document.hidden || !B) return;
  try {
