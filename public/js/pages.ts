/* ================================================================
   SHARED PAGE RUNTIME — ENERGY & AGRI pages.
   Same design system as home (terminal.css), same provenance rules.
   ================================================================ */
(function () {
'use strict';
...
const C = window.PAGE_CONFIG; // { api, sym, heroUnit }
...
/* ---------- renderers ---------- */
function renderTape() {
  const h = (D.tape || []).map(t => {
    const ch = t.changePct ?? 0, dg = t.price > 500 ? 1 : (t.price > 20 ? 2 : 3);
    return `<span class="tg"><span class="k">${esc(t.symbol)}</span><span class="v">${fmt(t.price, dg)}</span><span class="c ${cls(ch)}">${ch >= 0 ? '▲' : '▼'}${sgn(ch)}${fmt(Math.abs(ch), 2)}%</span></span>`;
  }).join('');
...
function renderHero() {
  ...
  $('#oo').textContent = fmt(g.open, 2); $('#oh').textContent = fmt(g.high, 2); $('#ol').textContent = fmt(g.low, 2); $('#op').textContent = fmt(g.prevClose, 2);
...
function renderMonitor() {
  $('#monTb').innerHTML = (D.monitor || []).filter(q => q.symbol !== 'XAU:USD' && q.symbol !== 'DXY').map(q => {
...
function renderSpot() {
  $('#spots').innerHTML = (D.spotlights || []).map(s =>
    `<div class="hc"><div class="t">${esc(s.symbol)}</div><div class="p">${esc(s.note || '')}</div><div class="c ${cls(s.changePct)}">${s.changePct >= 0 ? '▲' : '▼'}${fmt(Math.abs(s.changePct), 2)}%</div><div class="p" style="margin-top:3px">${fmt(s.price, 2)}</div></div>`).join('');
  $('#ratios').innerHTML = (D.ratios || []).map(r =>
    `<div class="hc"><div class="t">${esc(r.label)}</div><div class="c gold">${fmt(r.value, 1)}</div><div class="p">${esc(r.unit)}</div></div>`).join('');
  $('#spreads').innerHTML = (D.spreads || []).map(s =>
...
function renderExtra() {
  const eb = $('#extraBody'); if (!eb) return;
  if (C.type === 'energy') {
    eb.innerHTML = (D.eia && D.eia.length)
      ? '<table class="tb"><thead><tr><th>EIA SERIES</th><th class="r">VALUE</th><th class="r">Δ</th><th class="r">AS OF</th></tr></thead><tbody>' +
        D.eia.map(r => `<tr><td>${esc(r.label)}</td><td class="r"><b>${fmt(r.value, 1)}</b> <span class="dim">${esc(r.unit)}</span></td><td class="r ${cls(-(r.change ?? 0))}">${r.change == null ? '—' : sgn(r.change) + fmt(r.change, 1)}</td><td class="r mut">${esc(r.asOf)}</td></tr>`).join('') +
        '</tbody></table><div class="footnote">EIA WEEKLY · SERIES IDS SHOWN IN SOURCE · STOCKS DRAW = BULLISH PRODUCT</div>'
      : '<div class="footnote">EIA NOT WIRED — check EIA_API_KEY + series IDs (verify at eia.gov)</div>';
  } else {
    let h = '';
    if (D.water) {
      ...
      if (D.water.nqH2o) {
        ...
        h += `<div class="bi"><span class="n">CA WATER INDEX</span><span class="${cls(chg)}">${sgn(chg)}${fmt(chg, 0)} $/AF</span><span class="gold" style="justify-self:end">${fmt(D.water.nqH2o.value, 0)}</span></div>`;
      }
...
function renderCal() {
  $('#calTb').innerHTML = (D.calendar || []).map(c =>
...
/* ---------- chart (compact candle + MA20 + vol + hover) ---------- */
...
function drawChart() {
  ...
  const cs = getComputedStyle(document.body); const gold = cs.getPropertyValue('--gold').trim(), up = cs.getPropertyValue('--up').trim(), dn = cs.getPropertyValue('--dn').trim(), grid = cs.getPropertyValue('--grid').trim(), mut = cs.getPropertyValue('--mut').trim();
  ...
  if (hoverX >= 0) { const i = Math.max(0, Math.min(n - 1, Math.floor((hoverX - padL) / cw))); const x = padL + cw * i + cw / 2; ctx.setLineDash([2, 3]); ctx.strokeStyle = mut; ctx.beginPath(); ctx.moveTo(x, padT); ctx.lineTo(x, H - axB); ctx.stroke(); ctx.setLineDash([]);
...
async function loadAll() {
  try { D = await getJSON(C.api); renderAll(); } catch (e) { const s = $('#status'); if (s) s.textContent = 'API ERROR — ' + String(e && e.message || e); }
  try { const r = await getJSON('/api/candles?sym=' + encodeURIComponent(C.sym) + '&tf=' + TF); CH = r.candles || []; drawChart(); } catch { }
}
loadAll();
